...
timeScale = pl_confAddedFlds{pl_timeScale};
...
if (pl_confAddedFlds{pl_SingleXAxisValue} == 1)         % single plot
    ...
    for pl = 1:numPlots
        ...
        ymin = pf_plotsAddedFlds{pl}{plt_yRange}(1);
        ymax = pf_plotsAddedFlds{pl}{plt_yRange}(2);


        ind = find((y >= ymin) & (y <= ymax));
            ...
            if (fldY == -2)
                ...
                elseif (fldY < 0)
                   ...
                   pause;
                    ...
                    ytemp = z(:, fldY);%z{fldY}(:, indInRange);
            end
            if (fldZ < 0)
...
%                zDelFld = z{fldDelZ}(:, indInRange);   
            ...
            plot(yt, zt,'Color',clr, 'LineStyle', styl,'LineWidth',width,...
