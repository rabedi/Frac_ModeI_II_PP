classdef pp_energyDispInterior
    ...
    methods
        ...
        function [objout, crackAngleDeg, valid] = UpdateBySegment(obj, Xs, delUsLocalCrd, TrcnsLocalCrd, delVsLocalCrd, scalars)
...
%             if ((numPoints == 1) || (len < 1e-30) || (isnan(orientation(1))))
%                 numPoints
%                 len
%                 delX
%                 xst = Xs{1}
%                 xen = Xs{numPoints}
%                 THROW('Invalid segment in pp_energyDispInterior\n');
%             end
        ...
        function val = string2number(obj, str, ind1, ind2, indFld)
            ...
            if ((contains(str, 'int_vsn')) || (contains(str, 'pow')))
                if (contains(str, 'rel'))
                    ...
                    return;
                elseif ((ind1 == 1) || (contains(str, '_0')) || (contains(str, '_n')))
                    val = obj.int_vsn_0;
                    return;
                elseif ((ind1 == 2) || (contains(str, '_1')) || (contains(str, '_t')))
                    val = obj.int_vsn_1;
                    return;
                else %if ((nargin < 3) || (ind1 <= 0))
                    val = obj.int_vsn;
                    return;
                end
            end
