classdef energyFrontClass < handle
    ...
    methods(Access = public)
        ...
        function [bool, numElements, dissErrorMax, interfaceErrorMax] = read(obj, filePath, computeNelementsErrorMax)
            if nargin < 3
                computeNelementsErrorMax = 0;
            end
            bool = 1;
            ...
            if (computeNelementsErrorMax)
               buf = 'none';
               while (strcmp(buf, 'counter') == 0)
                   buf = fscanf(fid, '%s', 1);
               end
               numElements = READ(fid,'i');
               buf = 'none';
               while (strcmp(buf, 'maxValue') == 0)
                   buf = fscanf(fid, '%s', 1);
               end
               dissErrorMax = READ(fid,'d');
               buf = 'none';
               while (strcmp(buf, 'maxValue') == 0)
                   buf = fscanf(fid, '%s', 1);
               end
               interfaceErrorMax = READ(fid,'d');
            end 
            fclose(fid);
        end
