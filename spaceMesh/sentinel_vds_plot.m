...
%        end 
        ...
        if ~isempty(jobs{i}{j}.xLab)
            xlabel(['$$ ',jobs{i}{j}.xLab,' $$'], 'Interpreter', 'latex', 'FontSize', afs);
        end
        if ~isempty(jobs{i}{j}.yLab)
            ylabel(['$$ ',jobs{i}{j}.yLab,' $$'], 'Interpreter', 'latex', 'FontSize', afs);
        end
        if ~isempty(Z)
           ...
           fileName = '';
            if globalDefaults.figureDefaults.save.active == 1
                ...
                try   
                    ...
                    legend(ax{i}{axno},legText{i}{axno},'Location',legLoc, 'Interpreter', 'latex', 'FontSize', lfs);
...
for i = 1:lenOut
    ...
    if lenName == 1
        ...
        switch fldName{1}
            case 'count'
                dat = vdsObj.var.count;
                mx = min(len, length(dat));
                for k = 1:mx
                    out(k,i) = dat(k);
                end
                continue;
            case 'measure'
                dat = vdsObj.var.measure;
                mx = min(len, length(dat));
                for k = 1:mx
                    out(k,i) = dat(k);
                end
            ...
            if (isSS > 1)
                try
                    if (strncmp(nm, 'zpr', 3) == 1) % raw data
                       dataPts = pp1r.getDataVectorByDataName(nm);
                       for k = 1:min(len, length(dataPts))
                            out(k, i) = dataPts(k);
                       end
%                       out(:,i) = datPts;
