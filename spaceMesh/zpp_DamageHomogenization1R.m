classdef zpp_DamageHomogenization1R % damage evolution for one run
    properties
        E = 1;
        nu = 0.3;
        %        rho = 1;
        ...
        rawData2IntegrateInTimeNames = {'zpr_power'}; % time is the coordinate of xVals in rawDat (in dynamic simulations time)
    ...
    methods
        ...
        function objout = Process(obj, rawDataIn)
            ...
            for i = 1:obj.numRawDamage + 1
                fld = flds_ini(i);
                indIni = nan;
                for pt = 1:obj.indexEnd
                    dmg = obj.pData.data{fld}{pt};
                    if (dmg > obj.damageInitiationZeroTol)
                        indIni = pt;
                        break;
                    end
                end
